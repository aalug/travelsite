...
Tested with data not related to the actual store."""
...
def test_category_db_fixture(
        db, db_fixture_setup, id, name, slug, is_active):
    """Test for Category model fixture."""
...
def test_db_category_insert_data(
        db, category_factory, slug, is_active):
    """Test for inserting data into a category factory."""
...
def test_product_db_fixture(db, db_fixture_setup, id, web_id, name, slug,
                            description, is_active, created_at, updated_at):
    result = models.Product.objects.get(id=id)
    result_created_at = result.created_at.strftime(DATE_FORMAT)
    result_updated_at = result.updated_at.strftime(DATE_FORMAT)
...
@pytest.mark.dbfixture
def test_db_product_insert_data(db, product_factory, category_factory):
    """Test for creating Product objects."""
...
def test_db_product_inventory_dataset(
        db, db_fixture_setup, id, sku, upc, product_type, product, brand, is_active,
        retail_price, store_price, sale_price, weight, created_at, updated_at):
    """Test for Product Inventory dataset."""
...
def test_db_product_inventory_insert_data(db, product_inventory_factory):
    """Test for inserting data, ProductInventory model."""
...
def test_db_product_type_insert_data(db, product_type_factory):
    new_type = product_type_factory.create(name='demo_type')
    assert new_type.name == 'demo_type'


def test_db_product_type_uniqueness_integrity(
        db, product_type_factory):
    """Test that Product Type has to be unique."""
    product_type_factory.create(name='not_unique')
    with pytest.raises(IntegrityError):
        product_type_factory.create(name='not_unique')


def test_db_brand_insert_data(db, brand_factory):
    new_brand = brand_factory.create(name='demo_brand')
    assert new_brand.name == 'demo_brand'


def test_db_brand_uniqueness_integrity(db, brand_factory):
    """Test that Brand has to be unique."""
...
def test_db_media_dataset(db, db_fixture_setup, id, product_inventory, image,
...
def test_db_media_insert_data(db, media_factory):
...
def test_db_stock_dataset(db, db_fixture_setup, id, product_inventory,
                          last_checked, units, units_sold):
...
def test_db_stock_insert_data(db, stock_factory):
...
def test_db_product_attribute_dataset(db, db_fixture_setup, id, name, description):
    result = models.ProductAttribute.objects.get(id=id)
    assert result.name == name
    assert result.description == description


def test_db_product_attribute_insert_data(db, product_attribute_factory):
...
def test_db_product_attribute_uniqueness_integrity(db, product_attribute_factory):
...
def test_db_product_attribute_dataset(db, db_fixture_setup, id, product_attribute, attribute_value):
    result = models.ProductAttributeValue.objects.get(id=1)
    assert result.product_attribute.id == 1
    assert result.attribute_value == '10'


def test_db_product_attribute_value_data(db, product_attribute_value_factory):
...
def test_db_insert_inventory_product_values(db, product_with_attribute_values_factory):
